...
class ClienteSelecionado {
  final String id;
  final String nome;
  const ClienteSelecionado({required this.id, required this.nome});
}

class SelecionarClienteSheet extends StatefulWidget {
  const SelecionarClienteSheet({super.key});

  @override
  State<SelecionarClienteSheet> createState() => _SelecionarClienteSheetState();
}

class _SelecionarClienteSheetState extends State<SelecionarClienteSheet>
    with SingleTickerProviderStateMixin {
  late final TabController _tab;
  final _cadKey = GlobalKey<_CadastrarClienteTabState>();

  ClienteSelecionado? _selecionado;
  ...
  bool _mostrarClientesWeb = false;
  ...
  void initState() {
    ...
    _tab.addListener(() async {
      ...
      if (!_podeCadastrar) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Sem permissão para cadastrar clientes.'),
            ),
          );
          setState(() {});
        }
      ...
      // evita ficar clicando enquanto valida
      if (_loadingPlan) {
        _tab.index = 0;
        if (mounted) {
  ...
  Future<void> _initScope() async {
    ...
    try {
      ...
      final canRegisterClientsSuppliers =
          (perms['canRegisterClientsSuppliers'] ?? false) == true;

      setState(() {
        _companyId = companyId.isEmpty ? null : companyId;
        _podeCadastrar = canRegisterClientsSuppliers;
        _motivoCadastroBloqueado =
            canRegisterClientsSuppliers
                ? null
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            Row(
              children: [
                const Text('Clientes Web', style: TextStyle(fontSize: 12)),
                Switch(
                  value: _mostrarClientesWeb,
                  onChanged: (v) {
                    setState(() => _mostrarClientesWeb = v);
                  },
                ),
            ...
            child:
                _tab.index == 0
                    ? FilledButton.icon(
                      ...
                      label: const Text('Adicionar este cliente'),
                    )
                    : FilledButton.icon(
                      onPressed:
                          !podeCadastrarAgora
...
class _CatalogoClientesTab extends StatefulWidget {
  final String companyId;
  final void Function(ClienteSelecionado c) onEscolhido;
  final ClienteSelecionado? selecionado;
  final bool mostrarClientesWeb;

  const _CatalogoClientesTab({
    required this.companyId,
    required this.onEscolhido,
    required this.selecionado,
    required this.mostrarClientesWeb,
  });

  @override
  State<_CatalogoClientesTab> createState() => _CatalogoClientesTabState();
}

class _CatalogoClientesTabState extends State<_CatalogoClientesTab> {
  final _fs = FirebaseFirestore.instance;
  ...
  Widget build(BuildContext context) {
    final query = _fs
        ...
        .where('isCliente', isEqualTo: true)
        .orderBy('nomeLower');
            ...
            builder: (context, snap) {
              ...
              final docs =
                  (snap.data?.docs ?? []).where((d) {
                    ...
                    if (!widget.mostrarClientesWeb && isWeb) {
                      return false;
                    }
...
class _CadastrarClienteTab extends StatefulWidget {
  final String companyId;
  final String? createdByUid;
  final void Function(ClienteSelecionado novo) onSalvo;
  final Future<bool> Function() requirePlanSlot;
  final Future<void> Function() onPlanRefreshed;

  const _CadastrarClienteTab({
    super.key,
    required this.companyId,
  ...
  @override
  State<_CadastrarClienteTab> createState() => _CadastrarClienteTabState();
}

class _CadastrarClienteTabState extends State<_CadastrarClienteTab> {
  ...
  String _perfilRelacionamento = 'cliente';
  ...
  Future<void> _salvar() async {
    ...
    final inherited =
        context.findAncestorStateOfType<_SelecionarClienteSheetState>();
    if (inherited != null && !inherited._podeCadastrar) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Sem permissão para cadastrar clientes.'),
    ...
    try {
      final dados = <String, dynamic>{
        ...
        'isCliente': true,
        'isFornecedor': _perfilRelacionamento == 'ambos',
        'nome': nome,
      ...
      if (mounted) {
        widget.onSalvo(ClienteSelecionado(id: ref.id, nome: nome));
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
