// lib/Dashboard/TopServicosDashboardScreen.dart
// Dashboard: Top Serviços Vendidos — MULTIEMPRESA
// Escopo: scopeUserId = (users.companyId ?? auth.uid)
// Índice recomendado: (companyId ASC, data ASC)  // ou (userId ASC, data ASC) se usar legado
...
class TopServicosDashboardScreen extends StatefulWidget {
  const TopServicosDashboardScreen({super.key});

  @override
  State<TopServicosDashboardScreen> createState() =>
      _TopServicosDashboardScreenState();
}

class _TopServicosDashboardScreenState
    extends State<TopServicosDashboardScreen> {
  ...
  static const String kItensServicos = 'itensServicos';
  static const String kStatus = 'status'; // << usado para ignorar 'cancelado'
  ...
  // quantos serviços mostrar no ranking/gráfico
  int _topN = 3;
  ...
  Future<void> _initScope() async {
    ...
    try {
      ...
      debugPrint(
        '[TopServicosDash] escopo: scopeUserId=$_scopeUserId (companyId=$_companyId uid=${u.uid})',
      );
    } catch (e) {
      setState(() {
        _loadingScope = false;
  ...
  Future<Map<String, dynamic>?> _loadCurrentUserRecord() async {
    ...
    // 2) fallback por emailKey (se você usar esse campo)
  ...
  Query<Map<String, dynamic>> _query() {
    // Índice necessário conforme o campo de escopo escolhido:
    // - companyId ASC, data ASC
    // - (ou) userId ASC, data ASC
  ...
  Widget build(BuildContext context) {
    ...
    if (_scopeError != null) {
      return Scaffold(
        appBar: AppBar(title: const Text('Dashboard • Top Serviços')),
        body: Center(
          child: Padding(
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Dashboard • Top Serviços'),
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        ...
        builder: (ctx, snap) {
          ...
          if (docs.isEmpty) {
            return _EmptyState(
              title: 'Sem vendas de serviços no período selecionado.',
              subtitle:
          ...
          // ======= Agregação =======
          final Map<String, _ServicoAgg> servicos = {};
          final Set<String> pedidosComServicos = {};

          for (final d in docs) {
            final m = d.data();
            final itens = (m[kItensServicos] as List?)?.cast<dynamic>() ?? [];
            if (itens.isEmpty) continue;

            bool teveServico = false;
            for (final it in itens) {
              ...
              final s = servicos.putIfAbsent(
                key,
                () => _ServicoAgg(key: key, nome: nome, unidade: unidade),
              );
              if (s.nome.isEmpty && nome.isNotEmpty) s.nome = nome;
              if (s.unidade.isEmpty && unidade.isNotEmpty) s.unidade = unidade;

              s.qtd += qtd;
              s.valor += total;
              s.vuAcum += vu > 0 ? vu * qtd : 0;
              s.vuQtd += vu > 0 ? qtd : 0;
              teveServico = true;
            }
            if (teveServico) pedidosComServicos.add(d.id);
          }

          // KPIs
          final quantidadeTotal = servicos.values.fold<double>(
            0,
            (a, e) => a + e.qtd,
          );
          final faturamento = servicos.values.fold<double>(
            0,
            (a, e) => a + e.valor,
          );
          final pedidosQtd = pedidosComServicos.length;
          final ticketUnit =
              quantidadeTotal > 0 ? (faturamento / quantidadeTotal) : 0.0;

          // Ordenações
          final lista = servicos.values.toList();
          if (_rankByValue) {
          ...
          final labels = <String>[
            for (final s in top) (s.nome.isEmpty ? s.key : s.nome),
          ];
            ...
            child: ListView(
              ...
              children: [
                  ...
                  builder: (context, cts) {
                    ...
                    final kpis = [
                      (
                        ...
                        label: 'Qtd. vendida',
                        value: _formatCompact(quantidadeTotal),
                        color: Colors.indigo,
                      ),
                      (
                        icon: Icons.request_quote_outlined,
                        label: 'Faturamento (serv.)',
                      ...
                      (
                        ...
                        value: _fmtMoeda(ticketUnit),
                        color: Colors.teal,
                      ),
                      (
                        icon: Icons.receipt_long_outlined,
                        label: 'Pedidos c/ serviços',
                        value: '$pedidosQtd',
                ...
                _SectionCard(
                  ...
                  title:
                      'Top $_topN serviços ${_rankByValue ? "(por Valor)" : "(por Quantidade)"}$periodoStr',
                  trailing: Row(
                ...
                _SectionCard(
                  ...
                  title:
                      'Serviços (${lista.length}${_rankByValue ? " • ordenado por Valor" : " • ordenado por Quantidade"})',
                  child: ListView.separated(
                    ...
                    itemBuilder: (_, i) {
                      final s = lista[i];
                      final ticketUnit =
                          s.vuQtd > 0 ? (s.vuAcum / s.vuQtd) : 0.0;
                      return ListTile(
                        ...
                        subtitle: Text(
                          [
                            'Qtd ${_formatNumber(s.qtd)}${s.unidade.isEmpty ? "" : " ${s.unidade}"}',
                            'Total ${_fmtMoeda(s.valor)}',
...
class _KpiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      child: Padding(
        padding: const EdgeInsets.fromLTRB(12, 10, 12, 10), // mais compacto
        child: Row(
          children: [
            Container(
              width: 38,
